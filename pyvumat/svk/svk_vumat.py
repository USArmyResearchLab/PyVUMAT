...
def compute_stress_loop(stretch, lame1, two_mu):
    ...
    num_points = stretch.shape[1]
    I = np.identity(3)
    U = np.zeros((3,3))
    E = np.zeros((3,3))
    S = np.zeros((3,3))
    stress_mat = np.zeros((3,3))
    stress = np.zeros((6,num_points))
    
    # Loop through material points and compute stress
    for i in range(num_points):
        U[0,0], U[1,1] = stretch[0,i], stretch[1,i]
        U[2,2], U[0,1] = stretch[2,i], stretch[3,i]
        U[1,2], U[2,0] = stretch[4,i], stretch[5,i]
        ...
        stress[0,i], stress[1,i] = stress_mat[0,0], stress_mat[1,1]
        stress[2,i], stress[3,i] = stress_mat[2,2], stress_mat[0,1]
        stress[4,i], stress[5,i] = stress_mat[1,2], stress_mat[2,0]
